...
- (instancetype)init
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(calendarUpdated:) name:@"CalendarUpdated" object:nil];
    ...
    [[EKEventStore sharedEventStore] requestAccessToEntityType:EKEntityTypeEvent completion:^(BOOL granted, NSError *error) {
        if (granted) {
            dispatch_async(dispatch_get_main_queue(), ^{
                NSDate *endDate = [[NSDate dateYesterday] endOfDay];
